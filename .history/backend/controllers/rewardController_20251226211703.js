...
exports.getPointsRanking = async (req, res) => {
  try {
    const limit = parsePositiveInt(req.query.limit, 10, false)
    const offset = parsePositiveInt(req.query.offset, 0, true)

    // 先获取所有积分数据，然后在应用层计算排名
    const sql = `
      SELECT 
    ...
    const allRanking = await sequelize.query(sql, {
      type: QueryTypes.SELECT
    })
    ...
    // 在应用层添加排名
    const rankingWithRank = allRanking.map((item, index) => ({
      ...item,
      rank: index + 1
    }))

    // 分页处理
    const paginatedRanking = rankingWithRank.slice(offset, offset + limit)
