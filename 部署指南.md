# 校园活动管理系统 - 云服务器部署指南

本文档详细说明如何将校园活动管理系统部署到云服务器上。

## 📋 目录

1. [服务器环境准备](#1-服务器环境准备)
2. [数据库配置](#2-数据库配置)
3. [后端部署](#3-后端部署)
4. [前端部署](#4-前端部署)
5. [使用 PM2 管理进程](#5-使用-pm2-管理进程)
6. [使用 Nginx 作为反向代理](#6-使用-nginx-作为反向代理)
7. [配置 HTTPS（可选）](#7-配置-https可选)
8. [防火墙配置](#8-防火墙配置)
9. [常见问题排查](#9-常见问题排查)

---

## 1. 服务器环境准备

### 1.1 系统要求

- **操作系统**: Ubuntu 20.04+ / CentOS 7+ / Debian 10+
- **Node.js**: >= 20.19.0 或 >= 22.12.0
- **MySQL**: >= 5.7 或 >= 8.0
- **内存**: 至少 2GB RAM
- **磁盘**: 至少 20GB 可用空间

### 1.2 安装 Node.js

#### Ubuntu/Debian:
```bash
# 更新系统包
sudo apt update && sudo apt upgrade -y

# 安装 Node.js 20.x
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt install -y nodejs

# 验证安装
node --version
npm --version
```

#### CentOS:
```bash
# 安装 Node.js 20.x
curl -fsSL https://rpm.nodesource.com/setup_20.x | sudo bash -
sudo yum install -y nodejs

# 验证安装
node --version
npm --version
```

### 1.3 安装 MySQL

#### Ubuntu/Debian:
```bash
sudo apt install -y mysql-server
sudo systemctl start mysql
sudo systemctl enable mysql
```

#### CentOS:
```bash
sudo yum install -y mysql-server
sudo systemctl start mysqld
sudo systemctl enable mysqld
```

#### 配置 MySQL:
```bash
# 运行安全配置脚本
sudo mysql_secure_installation

# 登录 MySQL
sudo mysql -u root -p

# 创建数据库和用户
CREATE DATABASE campus_activities CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'campus_user'@'localhost' IDENTIFIED BY 'your_strong_password';
GRANT ALL PRIVILEGES ON campus_activities.* TO 'campus_user'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```

### 1.4 安装 Nginx

```bash
# Ubuntu/Debian
sudo apt install -y nginx

# CentOS
sudo yum install -y nginx

# 启动并设置开机自启
sudo systemctl start nginx
sudo systemctl enable nginx
```

### 1.5 安装 PM2（进程管理工具）

```bash
sudo npm install -g pm2
```

---

## 2. 数据库配置

### 2.1 上传并导入数据库

```bash
# 在服务器上创建项目目录
sudo mkdir -p /var/www/activities_management
sudo chown -R $USER:$USER /var/www/activities_management

# 上传项目文件到服务器（使用 scp 或 git）
# 如果使用 git:
cd /var/www/activities_management
git clone <your-repo-url> .

# 导入数据库
mysql -u campus_user -p campus_activities < backend/database/activity_management.sql
```

### 2.2 验证数据库导入

```bash
mysql -u campus_user -p campus_activities -e "SHOW TABLES;"
```

---

## 3. 后端部署

### 3.1 安装后端依赖

```bash
cd /var/www/activities_management/backend
npm install --production
```

### 3.2 配置环境变量

创建 `.env` 文件：

```bash
cd /var/www/activities_management/backend
nano .env
```

添加以下内容（根据实际情况修改）：

```env
# 数据库配置
DB_HOST=localhost
DB_PORT=3306
DB_NAME=campus_activities
DB_USER=campus_user
DB_PASSWORD=your_strong_password

# JWT 配置
JWT_SECRET=your_very_secure_jwt_secret_key_change_this_in_production
JWT_EXPIRES_IN=7d

# 服务器配置
PORT=3000
NODE_ENV=production

# CORS 配置（替换为你的域名）
CORS_ORIGIN=https://yourdomain.com
```

**重要提示**：
- 将 `JWT_SECRET` 改为一个强随机字符串
- 将 `CORS_ORIGIN` 改为你的实际域名
- 确保 `.env` 文件权限安全：`chmod 600 .env`

### 3.3 修改后端 CORS 配置

编辑 `backend/app.js`，修改 CORS 配置以允许生产域名：

```javascript
app.use(
  cors({
    origin: (origin, callback) => {
      // 允许的域名列表
      const allowedOrigins = [
        'https://yourdomain.com',
        'https://www.yourdomain.com',
        process.env.CORS_ORIGIN
      ].filter(Boolean);
      
      if (!origin || allowedOrigins.includes(origin)) {
        return callback(null, true);
      }
      return callback(new Error('Not allowed by CORS'));
    },
    credentials: true
  })
);
```

### 3.4 创建上传目录

```bash
mkdir -p /var/www/activities_management/backend/uploads
chmod 755 /var/www/activities_management/backend/uploads
```

### 3.5 使用 PM2 启动后端

```bash
cd /var/www/activities_management/backend

# 创建 PM2 配置文件
cat > ecosystem.config.js << EOF
module.exports = {
  apps: [{
    name: 'activities-backend',
    script: 'app.js',
    instances: 1,
    exec_mode: 'fork',
    env: {
      NODE_ENV: 'production',
      PORT: 3000
    },
    error_file: './logs/err.log',
    out_file: './logs/out.log',
    log_date_format: 'YYYY-MM-DD HH:mm:ss Z',
    merge_logs: true,
    autorestart: true,
    max_memory_restart: '1G'
  }]
}
EOF

# 创建日志目录
mkdir -p logs

# 启动应用
pm2 start ecosystem.config.js

# 保存 PM2 配置
pm2 save

# 设置 PM2 开机自启
pm2 startup
# 执行上面命令输出的命令（通常是 sudo env PATH=... pm2 startup systemd -u $USER --hp $HOME）
```

### 3.6 验证后端运行

```bash
# 查看 PM2 状态
pm2 status

# 查看日志
pm2 logs activities-backend

# 测试 API
curl http://localhost:3000/api/health
```

---

## 4. 前端部署

### 4.1 安装前端依赖

```bash
cd /var/www/activities_management/校园活动管理系统
npm install
```

### 4.2 配置环境变量

创建 `.env.production` 文件：

```bash
cd /var/www/activities_management/校园活动管理系统
nano .env.production
```

添加以下内容（替换为你的实际域名）：

```env
VITE_API_BASE_URL=https://yourdomain.com/api
```

### 4.3 构建前端项目

```bash
cd /var/www/activities_management/校园活动管理系统
npm run build
```

构建完成后，会在 `dist` 目录生成静态文件。

### 4.4 配置 Nginx 托管前端

创建 Nginx 配置文件：

```bash
sudo nano /etc/nginx/sites-available/activities_management
```

添加以下配置（替换为你的实际域名和路径）：

```nginx
server {
    listen 80;
    server_name yourdomain.com www.yourdomain.com;
    
    # 前端静态文件
    root /var/www/activities_management/校园活动管理系统/dist;
    index index.html;

    # 前端路由支持（Vue Router history 模式）
    location / {
        try_files $uri $uri/ /index.html;
    }

    # 后端 API 代理
    location /api {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }

    # WebSocket 支持（用于聊天功能）
    location /ws {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    }

    # 上传的图片文件（优先匹配，允许浏览器缓存以提升加载速度）
    location ~* ^/uploads/.*\.(jpg|jpeg|png|gif|webp|svg)$ {
        proxy_pass http://localhost:3000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        # 允许浏览器缓存图片 30 天，大幅提升加载速度
        expires 30d;
        add_header Cache-Control "public, max-age=2592000";
        add_header X-Content-Type-Options "nosniff";
        
        # 启用 sendfile 和优化
        proxy_buffering on;
        proxy_buffer_size 4k;
        proxy_buffers 8 4k;
    }

    # 其他上传文件（不缓存）
    location ^~ /uploads {
        proxy_pass http://localhost:3000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        add_header Cache-Control "no-cache, no-store, must-revalidate";
    }

    # 静态资源缓存（^~ 前缀会阻止此规则匹配 /uploads 路径）
    location ~* \.(jpg|jpeg|png|gif|ico|css|js|svg|woff|woff2|ttf|eot)$ {
        expires 1y;
        add_header Cache-Control "public, immutable";
    }

    # 日志
    access_log /var/log/nginx/activities_access.log;
    error_log /var/log/nginx/activities_error.log;
}
```

启用配置：

```bash
# 创建符号链接
sudo ln -s /etc/nginx/sites-available/activities_management /etc/nginx/sites-enabled/

# 测试 Nginx 配置
sudo nginx -t

# 重载 Nginx
sudo systemctl reload nginx
```

---

## 5. 使用 PM2 管理进程

### 5.1 常用 PM2 命令

```bash
# 查看所有应用状态
pm2 status

# 查看日志
pm2 logs activities-backend

# 重启应用
pm2 restart activities-backend

# 停止应用
pm2 stop activities-backend

# 删除应用
pm2 delete activities-backend

# 查看应用详情
pm2 show activities-backend

# 监控
pm2 monit
```

### 5.2 更新部署

当需要更新代码时：

```bash
# 1. 拉取最新代码
cd /var/www/activities_management
git pull

# 2. 更新后端依赖（如果需要）
cd backend
npm install --production

# 3. 重启后端
pm2 restart activities-backend

# 4. 更新前端
cd ../校园活动管理系统
npm install
npm run build

# 5. 重载 Nginx
sudo systemctl reload nginx
```

---

## 6. 使用 Nginx 作为反向代理

Nginx 配置已在第 4.4 节中完成。如果需要调整，可以修改 `/etc/nginx/sites-available/activities_management`。

### 6.1 优化 Nginx 配置

可以添加以下优化配置：

```nginx
# 在 server 块中添加
client_max_body_size 50M;  # 允许上传大文件
gzip on;
gzip_vary on;
gzip_min_length 1024;
gzip_types text/plain text/css text/xml text/javascript application/x-javascript application/xml+rss application/json;
```

---

## 7. 配置 HTTPS（可选但强烈推荐）

### 7.1 安装 Certbot

```bash
# Ubuntu/Debian
sudo apt install -y certbot python3-certbot-nginx

# CentOS
sudo yum install -y certbot python3-certbot-nginx
```

### 7.2 获取 SSL 证书

```bash
sudo certbot --nginx -d yourdomain.com -d www.yourdomain.com
```

按照提示完成配置，Certbot 会自动配置 Nginx 使用 HTTPS。

### 7.3 自动续期

Certbot 会自动创建定时任务，证书会在到期前自动续期。

验证自动续期：

```bash
sudo certbot renew --dry-run
```

---

## 8. 防火墙配置

### 8.1 配置 UFW（Ubuntu/Debian）

```bash
# 允许 SSH
sudo ufw allow 22/tcp

# 允许 HTTP
sudo ufw allow 80/tcp

# 允许 HTTPS
sudo ufw allow 443/tcp

# 启用防火墙
sudo ufw enable

# 查看状态
sudo ufw status
```

### 8.2 配置 firewalld（CentOS）

```bash
# 允许 HTTP
sudo firewall-cmd --permanent --add-service=http

# 允许 HTTPS
sudo firewall-cmd --permanent --add-service=https

# 重载防火墙
sudo firewall-cmd --reload

# 查看状态
sudo firewall-cmd --list-all
```

**注意**：不要开放 3000 端口，后端应该只通过 Nginx 访问。

---

## 9. 常见问题排查

### 9.1 后端无法启动

```bash
# 检查 PM2 日志
pm2 logs activities-backend --lines 100

# 检查数据库连接
cd /var/www/activities_management/backend
node test-db.js

# 检查端口占用
sudo netstat -tlnp | grep 3000
```

### 9.2 前端无法访问

```bash
# 检查 Nginx 状态
sudo systemctl status nginx

# 检查 Nginx 错误日志
sudo tail -f /var/log/nginx/activities_error.log

# 检查前端文件是否存在
ls -la /var/www/activities_management/校园活动管理系统/dist
```

### 9.3 API 请求失败

1. 检查后端是否运行：`pm2 status`
2. 检查 CORS 配置是否正确
3. 检查 Nginx 代理配置
4. 查看浏览器控制台和网络请求

### 9.4 数据库连接失败

```bash
# 测试数据库连接
mysql -u campus_user -p campus_activities

# 检查 MySQL 服务状态
sudo systemctl status mysql

# 检查防火墙是否阻止了连接
```

### 9.5 文件上传失败或上传文件无法访问

```bash
# 检查上传目录权限
ls -la /var/www/activities_management/backend/uploads

# 确保目录可写
chmod 755 /var/www/activities_management/backend/uploads

# 检查上传文件是否存在
ls -la /var/www/activities_management/backend/uploads/文件名.jpg
```

**如果出现 Nginx 错误：`open() "/var/www/activities_management/校园活动管理系统/dist/uploads/xxx.jpg" failed (2: No such file or directory)`**

这是因为 Nginx 配置中 `/uploads` 路径被静态资源缓存规则拦截了。解决方法：

1. 编辑 Nginx 配置文件：
```bash
sudo nano /etc/nginx/sites-available/activities_management
```

2. 确保 `/uploads` location 块使用 `^~` 前缀（优先匹配）：
```nginx
# 上传文件代理（使用 ^~ 确保优先匹配）
location ^~ /uploads {
    proxy_pass http://localhost:3000;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Proto $scheme;
}
```

3. 测试并重载配置：
```bash
sudo nginx -t
sudo systemctl reload nginx
```

### 9.6 WebSocket 连接失败

检查 Nginx 配置中的 `/ws` 位置块是否正确配置了 WebSocket 代理。

### 9.7 图片加载很慢

图片加载慢可能由以下原因造成：

#### 1. 检查图片文件大小
```bash
# 查看上传目录中的图片大小
ls -lh /var/www/activities_management/backend/uploads/*.jpg | head -10

# 如果图片过大（> 2MB），建议压缩
```

#### 2. 优化 Nginx 配置（已在上面的配置中优化）
确保图片文件启用了浏览器缓存，这样可以大幅提升二次加载速度。

#### 3. 检查服务器资源
```bash
# 查看 CPU 和内存使用情况
top
# 或
htop

# 查看磁盘 I/O
iostat -x 1

# 查看网络带宽
iftop
```

#### 4. 图片优化建议

**前端优化**：
- 使用图片懒加载（lazy loading）
- 使用 WebP 格式（更小的文件大小）
- 压缩图片后再上传

**服务器优化**：
- 如果图片文件很大，考虑使用 CDN
- 启用 HTTP/2（如果使用 HTTPS）
- 检查服务器带宽是否充足

#### 5. 测试图片加载速度
```bash
# 测试单个图片的响应时间
time curl -o /dev/null -s -w "%{time_total}\n" http://124.70.221.193/uploads/图片名.jpg

# 查看 Nginx 访问日志中的响应时间
sudo tail -f /var/log/nginx/activities_access.log | grep uploads
```

#### 6. 如果服务器配置较低
- **最低配置**：1核1G（可能较慢）
- **推荐配置**：2核2G 或更高
- **优化方案**：
  - 使用图片 CDN（如阿里云 OSS、腾讯云 COS）
  - 压缩图片文件
  - 使用 WebP 格式
  - 启用浏览器缓存（已在配置中优化）

---

## 10. 安全建议

1. **定期更新系统**：
   ```bash
   sudo apt update && sudo apt upgrade -y
   ```

2. **使用强密码**：确保数据库密码和 JWT_SECRET 足够强

3. **限制 SSH 访问**：使用密钥认证，禁用密码登录

4. **定期备份数据库**：
   ```bash
   # 创建备份脚本
   mysqldump -u campus_user -p campus_activities > backup_$(date +%Y%m%d).sql
   ```

5. **监控日志**：定期检查应用日志和系统日志

6. **使用环境变量**：不要在代码中硬编码敏感信息

---

## 11. 性能优化建议

1. **启用 Nginx 缓存**：对静态资源启用缓存
2. **使用 CDN**：将静态资源放到 CDN
3. **数据库优化**：添加必要的索引
4. **启用 Gzip 压缩**：已在 Nginx 配置中启用
5. **使用 Redis**：缓存热点数据（可选）

---

## 12. 部署检查清单

- [ ] Node.js 和 npm 已安装
- [ ] MySQL 已安装并配置
- [ ] 数据库已创建并导入数据
- [ ] 后端 `.env` 文件已配置
- [ ] 后端依赖已安装
- [ ] PM2 已安装并配置
- [ ] 后端服务正常运行
- [ ] 前端依赖已安装
- [ ] 前端已构建
- [ ] Nginx 已安装并配置
- [ ] 域名 DNS 已解析
- [ ] 防火墙已配置
- [ ] HTTPS 已配置（可选）
- [ ] 测试所有功能正常

---

## 13. 快速部署脚本示例

可以创建一个自动化部署脚本：

```bash
#!/bin/bash
# deploy.sh

echo "开始部署校园活动管理系统..."

# 更新代码
cd /var/www/activities_management
git pull

# 更新后端
cd backend
npm install --production
pm2 restart activities-backend

# 更新前端
cd ../校园活动管理系统
npm install
npm run build

# 重载 Nginx
sudo systemctl reload nginx

echo "部署完成！"
```

使用：
```bash
chmod +x deploy.sh
./deploy.sh
```

---

## 联系与支持

如遇到问题，请检查：
1. 日志文件
2. 系统资源使用情况
3. 网络连接
4. 配置文件是否正确

祝部署顺利！🎉

